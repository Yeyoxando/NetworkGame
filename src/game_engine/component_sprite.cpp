...
Sprite::Sprite(GameObject& gameobject, const Sprite& other){
  ...
  //Provisional until sprite handle management is done
  memset(img_name_, '\0', 128);
  strcpy(img_name_, other.img_name_);
  pivot_selection_ = 8;

  char src[128];
  memset(src, '\0', sizeof(default_sprite_folder));
  strcpy(src, default_sprite_folder);
  strcat(src, img_name_);
  img_ = IMG_Load(src);
...
Sprite::Sprite(GameObject& gameobject, int width, int height, const unsigned char* buffer){
  ...
  gameobject_ = &gameobject;
  pivot_point_ = kPivotPoint_Center;
  img_ = SDL_CreateRGBSurfaceWithFormatFrom((void*)buffer, width, height,
...
Sprite::~Sprite(){
  switch(origin_){
    case kSpriteOrigin_Null:{
      break;
    }
    case kSpriteOrigin_Image:{
      SDL_FreeSurface(img_);
      SDL_DestroyTexture(texture_);
      break;
    }
    case kSpriteOrigin_Buffer:{
      SDL_FreeSurface(img_);
      SDL_DestroyTexture(texture_);
      break;
    }
